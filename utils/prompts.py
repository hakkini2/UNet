...
def get_prediction_by_prompt(ground_truth_mask, predictor, prompt_type):
    if prompt_type == "naive_point" or prompt_type == "point" or prompt_type == "furthest_from_edges_point":
        pred, _, _ = get_point_prompt_prediction(ground_truth_mask, predictor, prompt_type)
    elif prompt_type == "box":
        (
            pred,
            _,
        ) = get_box_prompt_prediction(ground_truth_mask, predictor)
    elif prompt_type == "one_box_with_points":
        pred, _, _, _ = get_box_with_points_prediction(ground_truth_mask, predictor)
    elif prompt_type == "box_and_point":
        pred, _, _, _ = get_box_and_point_prompt_prediction(ground_truth_mask, predictor, background_point=False)
    elif prompt_type == "box_and_background_point":
        pred, _, _, _ = get_box_and_point_prompt_prediction(ground_truth_mask, predictor, background_point=True)
    elif prompt_type == "box_and_then_point":
        # Predict using a box, then based on the box prediction choose a point per cluster
        pred, _, _, _ = get_box_then_point_prompt_prediction(ground_truth_mask, predictor, point_type="fg")
    elif prompt_type == "box_and_then_background_point":
        # Predict using a box, then based on the box prediction choose a point per cluster
        pred, _, _, _ = get_box_then_point_prompt_prediction(ground_truth_mask, predictor, point_type="bg")
    elif prompt_type == "box_and_then_fg/bg_point":
        pred, _, _, _ = get_box_then_point_prompt_prediction(ground_truth_mask, predictor, point_type="fg/bg")
...
def get_point_prompt_prediction(ground_truth_mask, predictor, point_type):
    ...
    if point_type == "naive_point":
        ...
        point_prompts_list, _ = compute_furthest_point_from_edges(
            ground_truth_mask
        )  # compute the point on foreground that is furthes from the edges
